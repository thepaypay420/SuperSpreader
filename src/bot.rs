 ...
 pub async fn run(settings: Settings, store: SqliteStore) -> Result<()> {
     ...
     let (selected_tx, selected_rx) = watch::channel::<Vec<crate::market_selector::SelectedMarket>>(Vec::new());
                 ...
                 match selector.select().await {
                     Ok(markets) => {
                         store
                             ...
                             .ok();
                         let _ = selected_tx.send(markets);
                     }
                     Err(e) => {
                         store
                             .upsert_runtime_status("scanner", "error", "scan_failed", Some(&e.to_string()), now_ts())
                             .ok();
                     }
                 }
             }
 ...
 async fn run_paper_trader(
     ...
     if settings.paper_reset_on_start {
         ...
         if let Ok(rows) = store.fetch_latest_positions(5000) {
             for r in rows {
                 let mid = r.get("market_id").and_then(|x| x.as_str()).unwrap_or("").to_string();
                 if mid.is_empty() {
                     continue;
                 }
                 let qty = r.get("position").and_then(|x| x.as_f64()).unwrap_or(0.0);
                 let avg = r.get("avg_price").and_then(|x| x.as_f64()).unwrap_or(0.0);
                 let realized = r.get("realized_pnl").and_then(|x| x.as_f64()).unwrap_or(0.0);
                 if qty == 0.0 && realized == 0.0 {
                     continue;
                 }
                 broker.positions.insert(mid, crate::paper_broker::Position { qty, avg_price: avg, realized_pnl: realized });
             }
             log::info!("paper_state.rehydrated positions={}", broker.positions.len());
         }
     }
 
     // Per-market quote state.
     let mut last_quote_ts: std::collections::HashMap<String, f64> = std::collections::HashMap::new();
     let mut last_fair: std::collections::HashMap<String, f64> = std::collections::HashMap::new();
     let mut last_imb_sign: std::collections::HashMap<String, i32> = std::collections::HashMap::new();
     ...
     let mut eval_tick = tokio::time::interval(std::time::Duration::from_secs(settings.eval_interval_secs));
     ...
     loop {
         tokio::select! {
             _ = loop_tick.tick() => {
                 ...
                 for m in &selected {
                     if let Some(tob) = feed.get(&m.market_id) {
                         trade_one_market(&settings, &store, &risk, &strat, &mut broker, m, &tob, now, &mut last_quote_ts, &mut last_fair, &mut last_imb_sign)?;
                     }
                 }
             ...
             _ = snap_tick.tick() => {
                 persist_snapshots(&store, &feed, &broker, selected_rx.borrow().as_slice())?;
             }
             _ = eval_tick.tick() => {
                 let selected = selected_rx.borrow().clone();
                 log_eval(&settings, &feed, &broker, &selected, start_ts);
                 persist_telemetry_markdown(&store, &feed, &broker, &selected).ok();
             }
             _ = arb_tick.tick() => {
                 let selected = selected_rx.borrow().clone();
                 try_event_basket_arb(&settings, &feed, &mut broker, &selected).ok();
             }
 ...
 fn try_event_basket_arb(
     ...
     for (_eid, ms) in by_event {
         ...
         for m in ms {
             if let Some(tob) = feed.get(&m.market_id) {
                 let (Some(b), Some(a)) = (tob.best_bid, tob.best_ask) else { continue; };
                 if a <= b {
                     continue;
                 }
         ...
         if sum_asks < 0.98 && buy_edge > (cost_bps / 10_000.0) * 1.5 {
             ...
             log::info!("arb.buy_basket sum_asks={:.3} edge={:.4} legs={}", sum_asks, buy_edge, legs.len());
             ...
             log::info!("arb.sell_basket sum_bids={:.3} edge={:.4} legs={}", sum_bids, sell_edge, legs.len());
         }
     }
 ...
 fn log_eval(settings: &Settings, feed: &crate::feed_handler::FeedState, broker: &PaperBroker, selected: &[SelectedMarket], start_ts: f64) {
     let now = now_ts();
     ...
     let open_orders: Vec<_> = broker.orders.values().filter(|o| o.status == "open").collect();
     ...
     for o in &open_orders {
         if let Some(tob) = feed.get(&o.market_id) {
             match o.side {
                 Side::Buy => {
                     if tob.best_bid.is_some_and(|b| (o.price - b).abs() < 1e-12) { touch_hits += 1.0; }
                 }
                 Side::Sell => {
                     if tob.best_ask.is_some_and(|a| (o.price - a).abs() < 1e-12) { touch_hits += 1.0; }
                 }
             }
         }
     }
     let time_at_touch = if open_n > 0.0 { touch_hits / open_n } else { 0.0 };
     ...
     let avg_spread_bps = if spread_n > 0.0 { spread_bps_sum / spread_n } else { 0.0 };
 ...
 fn trade_one_market(
     ...
     tob: &Tob,
     now: f64,
     last_quote_ts: &mut std::collections::HashMap<String, f64>,
     last_fair: &mut std::collections::HashMap<String, f64>,
     last_imb_sign: &mut std::collections::HashMap<String, i32>,
 ) -> Result<()> {
     let (Some(bid), Some(ask)) = (tob.best_bid, tob.best_ask) else { return Ok(()); };
     if ask <= bid {
         return Ok(());
     }
     ...
     let min_profitable_spread_bps = 1.5 * settings.cost_bps();
     ...
     let is_active_market = tob.updates_ewma_per_min >= settings.min_updates_min;
 
     let decision = risk.can_quote(tob, now, is_active_market, min_profitable_spread_bps);
     if !decision.ok {
         // Pull orders if we can't trust the feed / risk says no.
         cancel_all_open_for_market(broker, &m.market_id)?;
         store.upsert_runtime_status("risk", "warn", decision.reason.unwrap_or("reject"), Some(&m.market_id), now).ok();
         return Ok(());
     }
 
     let inv_qty = broker.position_qty(&m.market_id);
     let (fair, fair_source) = match strat.compute_fair(tob, tob.last_trade_ema) {
         Some(x) => x,
         None => return Ok(()),
     };
 
     // Quote update conditions.
     let prev_ts = last_quote_ts.get(&m.market_id).copied().unwrap_or(0.0);
     let prev_fair = last_fair.get(&m.market_id).copied().unwrap_or(fair);
     let prev_sign = last_imb_sign.get(&m.market_id).copied().unwrap_or(0);
     let sign = if imbalance > 0.05 { 1 } else if imbalance < -0.05 { -1 } else { 0 };
 
     let should_requote = (now - prev_ts) >= 0.10
         && ( (fair - prev_fair).abs() >= settings.mm_reprice_threshold || sign != prev_sign );
     ...
     let _fills = broker.simulate_fills_for_market(&m.market_id, tob, activity_score)?;
     ...
     if imbalance.abs() > 0.3 && spread_bps >= min_profitable_spread_bps {
         ...
         let sz = (settings.base_order_size * 0.5).max(settings.base_order_size.min(1.0));
         let _ = broker.execute_ioc(&m.market_id, side, px, sz, "snipe", tob)?;
     }
 
     if !should_requote {
         // Still persist quote telemetry (helps dashboard explain decisions).
         store.insert_quote_snapshot(
             now,
             &m.market_id,
             m.event_id.as_deref().unwrap_or_else(|| "event:unknown"),
             tob.best_bid,
             tob.best_ask,
             Some(mid),
             Some(fair),
     ...
     cancel_stale_for_market(broker, &m.market_id, now, settings.mm_min_quote_life_secs)?;
 
     // Compute and place grid.
     let intents = strat.quote_grid(fair, inv_qty, imbalance, tob.updates_ewma_per_min);
     ...
     for qi in intents {
         ...
         // Inventory guardrail.
         if qi.side == Side::Buy && (inv_qty + qi.size) > settings.max_inventory_usd {
             continue;
         }
         if qi.side == Side::Sell && (inv_qty - qi.size) < -settings.max_inventory_usd {
             continue;
         }
 
         // Avoid duplicate prices on same side.
         if has_open_order_at(broker, &m.market_id, qi.side, qi.price) {
             continue;
         }
 
         let oid = broker.place_limit(&m.market_id, qi.side, qi.price, qi.size, "mm")?;
         let _ = oid;
         match qi.side {
     ...
     last_quote_ts.insert(m.market_id.clone(), now);
     last_fair.insert(m.market_id.clone(), fair);
     last_imb_sign.insert(m.market_id.clone(), sign);
 ...
 fn cancel_stale_for_market(broker: &mut PaperBroker, market_id: &str, now: f64, min_life: f64) -> Result<()> {
     let ids: Vec<String> = broker
         .orders
         .iter()
         .filter(|(_, o)| o.market_id == market_id && o.status == "open" && (now - o.created_ts) >= min_life)
         .map(|(id, _)| id.clone())
         .collect();
     for id in ids {
         broker.cancel(&id)?;
 ...
 fn persist_snapshots(store: &SqliteStore, feed: &crate::feed_handler::FeedState, broker: &PaperBroker, selected: &[SelectedMarket]) -> Result<()> {
     let now = now_ts();
     let mut total_u = 0.0;
     let total_r = broker.realized_pnl_total();
 
     let mut event_by_market: std::collections::HashMap<&str, &str> = std::collections::HashMap::new();
     for m in selected {
         if let Some(eid) = m.event_id.as_deref() {
     ...
     for (mid, p) in broker.positions.iter() {
         ...
         store.insert_position_snapshot(
             now,
             mid,
             event_by_market.get(mid.as_str()).copied().unwrap_or_else(|| "event:unknown"),
             p.qty,
             p.avg_price,
             mark,
             u,
